...
describe('CalculateMonitorStatisticsJob', function () {
    describe('handle', function () {
        it('processes all public monitors sequentially in batches', function () {
            Queue::fake();
            ...
            Queue::assertPushed(CalculateMonitorStatisticsJob::class, function ($pushed) use ($monitor) {
                $ids = (new ReflectionProperty($pushed, 'monitorIds'))->getValue($pushed);

                return is_array($ids) && in_array($monitor->id, $ids, true);
            });
        });
        ...
        it('calculates statistics for a batch of monitors', function () {
            $monitor1 = Monitor::factory()->create([
                'is_public' => true,
                'uptime_check_enabled' => true,
            ]);
            ...
            MonitorUptimeDaily::updateOrInsert(
                ['monitor_id' => $monitor2->id, 'date' => now()->toDateString()],
                ['uptime_percentage' => 100, 'total_checks' => 1, 'failed_checks' => 0]
            );

            $job = new CalculateMonitorStatisticsJob([$monitor1->id, $monitor2->id]);
            $job->handle();

            $this->assertDatabaseHas('monitor_statistics', ['monitor_id' => $monitor1->id]);
            $this->assertDatabaseHas('monitor_statistics', ['monitor_id' => $monitor2->id]);
        });
