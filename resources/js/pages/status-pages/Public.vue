...
useMonitorStatusStream({
    ...
    onStatusChange: (change) => {
        ...
        if (latestHistory.value[change.monitor_id])
            latestHistory.value[change.monitor_id] = { ...latestHistory.value[change.monitor_id]!, uptime_status: change.new_status };
    },
});
...
const getDomainExpirationLabel = (d?: string | null) => {
    if (!d) return '';
    const days = Math.ceil((new Date(d).getTime() - Date.now()) / 86400000);
    if (days < 0) return 'Domain expired';
    if (days === 0) return 'Domain expires today';
    return `Domain ${days}d`;
};
    ...
    >
        <template #header-left>
            <div class="flex min-w-0 items-center gap-3">
                <div
                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 p-2 text-white shadow-md shadow-blue-500/20"
                >
                ...
                <div class="min-w-0">
                    ...
                    <p class="line-clamp-1 hidden text-xs text-gray-500 md:block dark:text-gray-400" :title="statusPage.description">
        ...
        <div
            class="mb-8 overflow-hidden rounded-3xl border p-6 shadow-sm backdrop-blur-md transition-all duration-300 sm:p-8"
            :class="[
        ...
        >
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="rounded-2xl border border-gray-200/80 bg-white/90 px-4 py-2 text-center shadow-sm backdrop-blur-sm dark:border-gray-800 dark:bg-gray-800/90"
                    >
                        <span class="block text-lg font-extrabold text-gray-900 dark:text-white">
                            {{ monitors.filter((m) => (latestHistory[m.id]?.uptime_status || m.uptime_status)?.toLowerCase() === 'up').length }} /
                            {{ monitors.length }}
                        </span>
                        <span class="text-[10px] font-semibold tracking-wider text-gray-400 uppercase">Services Online</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Services List -->
        <div
            class="overflow-hidden rounded-3xl border border-gray-200/80 bg-white/80 shadow-sm backdrop-blur-sm dark:border-gray-800/80 dark:bg-gray-900/80"
        >
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-800">
                <h3 class="text-base font-bold text-gray-900 dark:text-white">Tracked Services</h3>
                <span class="text-xs font-medium text-gray-400">{{ monitors.length }} Monitored Components</span>
            </div>

            <div v-if="monitorsError" class="p-12 text-center">
                <Icon name="alertCircle" class="mx-auto h-12 w-12 text-rose-500" />
                <h4 class="mt-3 text-lg font-bold text-gray-900 dark:text-white">
                    {{ monitorsError === 'Status page not found' ? '404 - Page Not Found' : 'Error Loading Status' }}
                </h4>
                <p class="mx-auto mt-1 max-w-md text-xs text-gray-500 dark:text-gray-400">
                    {{ monitorsError === 'Status page not found' ? 'This status page does not exist or has been removed.' : monitorsError }}
                </p>
            </div>
            ...
            <div v-else class="divide-y divide-gray-100 dark:divide-gray-800">
                <div
                    ...
                    :key="monitor.id"
                    class="group p-5 transition-colors hover:bg-gray-50/50 sm:p-6 dark:hover:bg-gray-800/30"
                >
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex min-w-0 items-center gap-3.5">
                            ...
                            <div
                                v-else
                                class="flex h-6 w-6 items-center justify-center rounded-lg bg-gray-100 font-mono text-[10px] font-bold text-gray-600 dark:bg-gray-800 dark:text-gray-400"
                            >
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    <Link
                                        :href="'/m/' + monitor.host"
                                        class="text-base font-bold text-gray-900 transition-colors hover:text-blue-600 dark:text-white dark:hover:text-blue-400"
                                    >
                                        {{ monitor.name }}
                                    </Link>
                                    <span
                                ...
                                <a
                                    class="block truncate text-xs text-gray-400 transition-colors hover:text-blue-500"
                                    :href="monitor.url"
                                    target="_blank"
                                    rel="noopener"
                                    >{{ monitor.url }}</a
                        ...
                        <div class="flex shrink-0 items-center justify-between gap-3 sm:justify-end">
                            <div class="text-left sm:text-right">
                                <div
                                    ...
                                    :class="getStatusTextColor(latestHistory[monitor.id]?.uptime_status || monitor.uptime_status)"
                                >
                                ...
                                :class="[
                                    ...
                                    (latestHistory[monitor.id]?.uptime_status || monitor.uptime_status) === 'up'
                                        ...
                                        : (latestHistory[monitor.id]?.uptime_status || monitor.uptime_status) === 'down'
                                          ? 'animate-ping bg-rose-500 ring-4 ring-rose-500/20'
