...
<template>
    ...
    >
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        <h1 class="truncate text-base font-extrabold text-gray-900 dark:text-white sm:text-xl uppercase tracking-tight">Status Wallboard</h1>
                        <span class="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-extrabold uppercase text-blue-600 dark:bg-blue-950/50 dark:text-blue-400">
                            {{ monitors.data.length }} Monitors
                        </span>
                    </div>
                    <p class="truncate text-xs font-semibold text-emerald-600 dark:text-emerald-400">
        ...
        <div class="mx-auto max-w-[1920px]">
            ...
            <div class="mb-8 flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-gray-200/80 bg-white/80 p-4 shadow-sm backdrop-blur-sm dark:border-gray-800/80 dark:bg-gray-900/80">
                <div class="flex flex-1 flex-wrap items-center gap-2.5 min-w-[280px]">
                    <!-- Search -->
                    <div class="relative flex-1 max-w-xs">
                        <Input
                            v-model="searchQuery"
                            placeholder="FILTER MONITORS..."
                            class="h-9 rounded-xl border-gray-200/80 bg-gray-50/50 px-8 text-[11px] font-bold uppercase tracking-wider backdrop-blur-sm dark:border-gray-800 dark:bg-gray-800/50"
                        />
                        <Icon name="search" class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" size="14" />
                    </div>

                    <!-- Sort Controls -->
                    <div class="flex items-center gap-1 rounded-xl border border-gray-200/80 bg-gray-100/80 p-1 dark:border-gray-800 dark:bg-gray-900">
                        <DropdownMenu>
                            <DropdownMenuTrigger as-child>
                                <Button variant="ghost" class="h-7 gap-2 rounded-lg px-2 text-[10px] font-bold uppercase tracking-widest hover:bg-white dark:hover:bg-gray-800">
                        ...
                        <Button 
                            variant="ghost" 
                            size="icon" 
                            class="h-7 w-7 rounded-lg hover:bg-white dark:hover:bg-gray-800"
                    ...
                    <div class="flex rounded-xl border border-gray-200/80 bg-gray-100/80 p-1 dark:border-gray-800 dark:bg-gray-900">
                        <button
                            ...
                            :class="[
                                'h-7 rounded-lg px-3 text-[10px] font-bold uppercase tracking-widest transition-all',
                                groupBy === group 
                                    ? 'bg-white text-blue-600 shadow-sm dark:bg-gray-800 dark:text-blue-400' 
                                    : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'
                            ]"
                    ...
                    <Link
                        ...
                        href="/login"
                        class="h-9 rounded-xl bg-blue-600 px-4 flex items-center text-[10px] font-bold uppercase tracking-widest text-white shadow-sm hover:bg-blue-700 active:scale-95 transition-all"
                    >
                        LOGIN
                    </Link>
                    ...
                    <template v-if="isAuthenticated">
                        <DropdownMenu>
                            <DropdownMenuTrigger as-child>
                                <Button
                                    variant="outline"
                                    class="h-9 rounded-xl px-4 text-[10px] font-bold uppercase tracking-widest border-gray-200/80 dark:border-gray-800"
                                >
                            ...
                            <DropdownMenuContent align="end" class="w-40 rounded-xl border border-gray-200/80 dark:border-gray-800">
                                <DropdownMenuItem as="a" href="/monitor/export/csv" class="text-[10px] font-bold uppercase tracking-widest">
                                    <Icon name="fileText" class="mr-2" size="12" /> CSV
                                </DropdownMenuItem>
                                <DropdownMenuItem as="a" href="/monitor/export/json" class="text-[10px] font-bold uppercase tracking-widest">
                        ...
                        <Button
                            ...
                            class="h-9 rounded-xl bg-blue-600 px-4 text-[10px] font-bold uppercase tracking-widest text-white shadow-sm hover:bg-blue-700 active:scale-95 transition-all"
                        >
                            ADD MONITOR
                        </Button>
                    </template>
