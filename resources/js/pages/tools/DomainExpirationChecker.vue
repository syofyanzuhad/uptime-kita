...
interface Props {
    ...
    initialResult?: DomainResult | null;
    appUrl: string;
}
...
const result = ref<DomainResult | null>(props.initialResult || null);
const loading = ref(false);
const errorMessage = ref('');

const pageTitle = 'Free Domain Expiration & WHOIS Checker - Uptime Kita';
const pageDescription = 'Check domain registration expiration dates, days remaining before renewal, and WHOIS/RDAP records instantly for any domain.';
const shareUrl = `${props.appUrl}/tools/domain-expiration`;
const shareText = 'Check domain expiration dates and WHOIS information for free with Uptime Kita!';
...
async function checkDomain() {
    ...
    try {
        const res = await fetch('/api/tools/domain-expiration', {
            method: 'POST',
            headers: {
        ...
        if (!res.ok) {
            errorMessage.value = data.message || 'Domain check failed.';
        } else {
            result.value = data;
            if (!data.ok) {
                errorMessage.value = data.error || 'Unable to inspect domain expiration.';
            }
        }
    } catch {
        errorMessage.value = 'Network error while checking domain expiration.';
    } finally {
        loading.value = false;
    }
}
...
<template>
    <PublicLayout :title="pageTitle" :description="pageDescription" :share-url="shareUrl" :share-text="shareText" container-class="max-w-4xl">
        ...
        <div class="mb-4 flex items-center gap-2 text-xs font-semibold text-gray-500 dark:text-gray-400">
            ...
            <span>/</span>
            <span class="text-gray-900 dark:text-white">Domain Expiration Checker</span>
        </div>

        <!-- Header -->
        <div class="mb-6 text-center sm:mb-8">
            <div
                class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 text-white shadow-md"
            >
                <Icon name="calendarClock" class="h-6 w-6" />
            </div>
            <h1 class="mt-3 text-2xl font-black tracking-tight text-gray-900 sm:text-3xl dark:text-white">Domain Expiration & WHOIS Checker</h1>
            <p class="mx-auto mt-1.5 max-w-xl text-xs text-gray-500 sm:text-sm dark:text-gray-400">
                Instantly check domain expiration dates, days remaining before renewal, and registration status via RDAP & WHOIS.
            </p>
        </div>
        ...
        <Card class="mb-6 rounded-3xl border border-gray-200/80 bg-white/80 shadow-sm backdrop-blur-sm dark:border-gray-800/80 dark:bg-gray-900/80">
            <CardContent class="p-4 sm:p-5">
                <form @submit.prevent="checkDomain" class="flex flex-col gap-2.5 sm:flex-row sm:items-center">
                    <div class="relative flex-1">
                        <Icon name="globe" class="pointer-events-none absolute top-1/2 left-3.5 h-4 w-4 -translate-y-1/2 text-gray-400" />
                        <input
                            v-model="domainInput"
                            type="text"
                            placeholder="Enter domain (e.g. google.com, github.com, mydomain.id)..."
                    ...
                    >
                        ...
                        <span>{{ loading ? 'Checking…' : 'Check Expiration' }}</span>
                    </button>
                </form>
                ...
                <div class="mt-3 flex flex-wrap items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <button
                        v-for="d in ['google.com', 'github.com', 'laravel.com', 'wikipedia.org']"
                        :key="d"
                        type="button"
                        @click="
                            domainInput = d;
        ...
        <div v-if="result && result.ok" class="space-y-4">
            ...
            >
                <div class="flex items-center gap-3.5">
                    <div
                        ...
                        :class="!result.is_expired ? 'bg-emerald-500' : 'bg-rose-500'"
                    >
                        <Icon :name="!result.is_expired ? 'check' : 'x'" class="h-6 w-6" />
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                ...
                                "
                            >
                                {{ !result.is_expired ? 'Active Registration' : 'Expired Domain' }}
                            </span>
                        </div>
                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                            Lookup protocol: RDAP / WHOIS • Checked in {{ result.elapsed_ms }}ms
                        </p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Card class="rounded-3xl border border-gray-200/80 bg-white/80 dark:border-gray-800/80 dark:bg-gray-900/80">
                    <CardContent class="p-4 sm:p-5">
                        <h3 class="mb-3 text-xs font-bold tracking-wider text-gray-400 uppercase">Expiration Timestamp</h3>
                        <dl class="space-y-2 text-xs">
                            <div class="flex justify-between border-b border-gray-100 pb-1.5 dark:border-gray-800">
                                ...
                                <dd class="font-mono font-medium text-gray-900 dark:text-white">{{ result.expires_at_formatted }}</dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-gray-500 dark:text-gray-400">ISO 8601:</dt>
                                <dd class="font-mono font-medium text-gray-900 dark:text-white">{{ result.expires_at }}</dd>
                            </div>
                        </dl>
                    </CardContent>
                </Card>
                ...
                <Card class="rounded-3xl border border-gray-200/80 bg-white/80 dark:border-gray-800/80 dark:bg-gray-900/80">
                    <CardContent class="p-4 sm:p-5">
                        <h3 class="mb-3 text-xs font-bold tracking-wider text-gray-400 uppercase">Status & Renewal</h3>
                        <dl class="space-y-2 text-xs">
                            <div class="flex justify-between border-b border-gray-100 pb-1.5 dark:border-gray-800">
                                <dt class="text-gray-500 dark:text-gray-400">Days Remaining:</dt>
                                <dd class="font-mono font-medium text-gray-900 dark:text-white">{{ result.days_remaining }} days</dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-gray-500 dark:text-gray-400">Status:</dt>
                                <dd class="font-medium text-emerald-600 dark:text-emerald-400">
                                    {{ !result.is_expired ? 'Active' : 'Expired / Pending Delete' }}
                                </dd>
                            </div>
                        </dl>
                    </CardContent>
            ...
            <div class="rounded-2xl border border-blue-200 bg-blue-50/70 p-4 text-center dark:border-blue-900/40 dark:bg-blue-950/30">
                <p class="text-xs font-bold text-blue-900 dark:text-blue-200">Never let your domain expire unexpectedly</p>
        ...
        <!-- Other Free Tools -->
        <RelatedTools current-slug="domain-expiration" :target-value="domainInput" />
    </PublicLayout>
</template>
