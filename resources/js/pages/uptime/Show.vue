...
const badgeUrl = computed(() => {
    ...
    return `${baseUrl}/badge/${monitorData.value.host}`;
});
...
const nextCheckCountdown = ref(getSecondsUntilNextCheck(monitorData.value.last_check_date, checkIntervalSeconds.value));
        ...
        <div class="py-8">
            <div class="mx-auto max-w-6xl space-y-6 sm:px-6 lg:px-8">
                    ...
                    <CardHeader>
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <img v-if="monitorData.favicon" :src="monitorData.favicon" :alt="`${domainName} favicon`" class="h-8 w-8 rounded" />
                                <div>
                    ...
                    <CardContent>
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <Icon :name="getStatusIcon(monitorData.uptime_status)" class="h-5 w-5" :class="statusColor" />
                                    <span :class="statusBgColor" class="rounded-full px-3 py-1 text-sm font-medium">
                                        {{ getStatusText(monitorData.uptime_status) }}
                                    </span>
                                </div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Last checked: {{ formatDate(monitorData.last_check_date) }}</p>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">Next check in {{ nextCheckCountdown }} seconds</p>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="space-y-1 text-sm">
                                    ...
                                    <p v-if="monitorData.uptime_status_last_change_date">
                                        <span class="text-gray-500">Status changed:</span>
                                        {{ formatDate(monitorData.uptime_status_last_change_date) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Failure Reason -->
                        <div
                            v-if="monitorData.uptime_check_failure_reason"
                            class="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-900/20"
                        >
                            <h4 class="mb-1 text-sm font-medium text-red-800 dark:text-red-200">Last Failure Reason</h4>
                ...
                <Card>
                    ...
                    <CardContent>
                        <TooltipProvider>
                            <div class="flex h-16 w-full items-end gap-[1px]">
                                <Tooltip v-for="(date, i) in last100Minutes" :key="i">
                                    <TooltipTrigger
                                        class="h-full cursor-pointer rounded transition-colors hover:opacity-80"
                                        :style="{
                                            width: 'calc(100% / 100)',
                                            background:
                                    ...
                                    <TooltipContent>
                                        <div class="text-xs whitespace-nowrap">
                                            ...
                                            <div>
                                                Status:
                                                <span v-if="getMinuteStatus(date)">{{ getStatusText(getMinuteStatus(date)!.uptime_status) }}</span>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div v-if="!histories || histories.length === 0" class="py-8 text-center text-gray-600 dark:text-gray-400">
                            <Icon name="inbox" class="mx-auto mb-4 h-12 w-12 text-gray-400" />
                                ...
                                <TableBody>
                                    <TableRow v-for="(history, index) in histories" :key="history.id">
                                        ...
                                        <TableCell>
                                            <div class="flex items-center gap-2">
                                                <Icon
                                                    :name="getStatusIcon(history.uptime_status)"
                                                    class="h-4 w-4"
                                                    :class="getHistoryStatusColor(history.uptime_status)"
                                                />
                                                <span
                                                    :class="getHistoryStatusBgColor(history.uptime_status)"
                ...
                <Card>
                    ...
                    <CardContent class="space-y-4">
                        <!-- Badge Preview -->
                        <div class="flex justify-center rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
                            <img :src="badgeUrl" :alt="`${domainName} uptime badge`" class="h-5" />
                        </div>
                        ...
                        <div class="border-t border-gray-200 pt-4 dark:border-gray-700">
                            <div class="mb-3 flex items-center space-x-2">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Usage Guide</span>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div class="space-y-2 text-sm">
                                    ...
                                    <ul class="ml-3 list-disc space-y-1 text-gray-600 dark:text-gray-400">
                                        <li>
                                            <code class="rounded bg-gray-100 px-1 dark:bg-gray-700">period</code> - Time period
                                            <div class="mt-0.5 ml-2 text-xs text-gray-500">
                                                <code class="bg-gray-100 dark:bg-gray-700">24h</code> (default),
                                                <code class="bg-gray-100 dark:bg-gray-700">7d</code>,
                                                <code class="bg-gray-100 dark:bg-gray-700">30d</code>,
                                                <code class="bg-gray-100 dark:bg-gray-700">90d</code>
                                            </div>
                                        </li>
                                        <li>
                                            <code class="rounded bg-gray-100 px-1 dark:bg-gray-700">style</code> - Badge style
                                            <div class="mt-0.5 ml-2 text-xs text-gray-500">
                                                <code class="bg-gray-100 dark:bg-gray-700">flat</code> (default),
                                                <code class="bg-gray-100 dark:bg-gray-700">flat-square</code>,
                                                <code class="bg-gray-100 dark:bg-gray-700">plastic</code>,
                                                <code class="bg-gray-100 dark:bg-gray-700">for-the-badge</code>
                                            </div>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <!-- Style Preview -->
                                <div class="space-y-2">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Style Preview:</span>
                            ...
                            <div class="mt-4 space-y-2">
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Example URLs:</span>
                                <div class="grid grid-cols-1 gap-2 text-xs md:grid-cols-3">
                                    <div class="rounded bg-gray-50 p-2 dark:bg-gray-800">
                                        ...
                                        <code class="break-all">{{ badgeUrl }}?period=7d</code>
                                    </div>
                                    <div class="rounded bg-gray-50 p-2 dark:bg-gray-800">
                                        <div class="mb-1 text-gray-500 dark:text-gray-400">30-day flat-square:</div>
                                        <code class="break-all">{{ badgeUrl }}?period=30d&amp;style=flat-square</code>
                                    </div>
                                    <div class="rounded bg-gray-50 p-2 dark:bg-gray-800">
                                        <div class="mb-1 text-gray-500 dark:text-gray-400">Large badge:</div>
                                        <code class="break-all">{{ badgeUrl }}?style=for-the-badge</code>
                                    </div>
                                </div>
                            </div>
                        </div>
