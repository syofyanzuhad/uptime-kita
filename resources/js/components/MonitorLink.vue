...
const ariaLabel = computed(() => {
    const status = props.monitor.uptime_status === 'up' ? 'operational' : props.monitor.uptime_status === 'down' ? 'down' : 'unknown status';
...
<template>
    <div :class="['flex items-center gap-2', className]">
        ...
        >
            {{ monitor.uptime_status === 'up' ? 'Operational' : monitor.uptime_status === 'down' ? 'Down' : 'Unknown' }}
        </span>
    </div>
</template>
