...
const cleanLabel = (label: string): string => {
    return label.replace('&laquo;', '').replace('&raquo;', '').replace('Previous', '').replace('Next', '').trim();
};
...
<template>
    <div :class="['flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between', className]">
        ...
        <!-- Pagination Links -->
        <nav
            v-if="getPaginationInfo().links && getPaginationInfo().links.length > 3"
        ...
        >
            <component
                ...
                :class="[
                    'relative inline-flex cursor-pointer items-center px-3 py-2 text-sm font-medium transition-colors duration-200',
                    // First link styling
            ...
            >
                <!-- Previous arrow -->
                <Icon v-if="isPrevious(link.label)" name="chevronLeft" class="h-4 w-4" aria-hidden="true" />
                ...
                <!-- Next arrow -->
                <Icon v-if="isNext(link.label)" name="chevronRight" class="h-4 w-4" aria-hidden="true" />
