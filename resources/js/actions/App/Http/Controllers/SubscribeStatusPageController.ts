 ...
 */
const SubscribeStatusPageController = (
...
SubscribeStatusPageController.url = (args: { path: string | number } | [path: string | number] | string | number, options?: RouteQueryOptions) => {
    ...
    return SubscribeStatusPageController.definition.url.replace('{path}', parsedArgs.path.toString()).replace(/\/+$/, '') + queryParams(options);
};
 ...
 * @route '/status/{path}/subscribe'
 */
SubscribeStatusPageController.post = (
    args: { path: string | number } | [path: string | number] | string | number,
    options?: RouteQueryOptions,
): RouteDefinition<'post'> => ({
    url: SubscribeStatusPageController.url(args, options),
